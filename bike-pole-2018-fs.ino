...
void setup()
{
  ...
  FastLED.addLeds<WS2812, DATA_PIN, RGB>(leds[2], NUM_LEDS);
...
void loop()
{
  ...
  for (int i = 0; i < NUM_LEDS; i++) {
    leds[2][i] = blend(leds[2][i], leds[0][i], percent);
    leds[2][i] = blend(leds[2][i], leds[1][i], 255 - percent);
  }
  FastLED.show();
}
